...
class Statistics:
    ...
    def getStudGivenAssig(self, assignID):
        ...
        '''
        x = self._control.filterGradesBy(None, assignID)
        stud = []
        for g in x:
            sg = StudentGradeDTO(self._control.student.findByID(g.studentID).name, g.grade)
            stud.append(sg)
        return sorted(stud, key=lambda student: student.name)
    ...
    def getLateStuds(self):
        ...
        for st in self._control.student.getAll():
            grAS = self._control.filterGradesBy(st.id)
            for gr in grAS:
                if gr.grade == 0.0:
    ...
    def getStudBestGrSchool(self):
        ...
        for st in students:
            stGra = self._control.filterGradesBy(st.id)
            kGr = 0
            avg = 0
            for gr in stGra:
                if gr.grade != 0.0:
        ...
        return sorted(bg, key=lambda x: x.grade, reverse=True)
    ...
    def getAssigByGrade(self):
        ...
        for a in assign:
            assGra = self._control.filterGradesBy(None, a.id)
            kGr = 0
            avg = 0
            for gr in assGra:
                if gr.grade != 0.0:
            ...
            if kGr != 0:
                avg /= kGr
                r.append(AssignmentGradeDTO(a, avg))
        return sorted(r, key=lambda x: x.grade, reverse=True)
